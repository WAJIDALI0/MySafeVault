...
export function ViewVaultItemDialog({ itemId, itemTitle, itemType, itemIsFavorite = false, itemDescription, open, onOpenChange }: ViewVaultItemDialogProps) {
    ...
    return (
      <div className="space-y-1.5 mb-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1 bg-slate-50 dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-lg p-3 text-sm text-slate-800 dark:text-slate-200 font-mono flex items-center justify-between group">
            ...
            <div className="flex items-center gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
              {isSecret && (
                <button 
                  ...
                  className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-md text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white transition-colors disabled:opacity-50"
              ...
              {isLink && (
                <a 
                  ...
                  className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-md text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white transition-colors"
                  title="Open link"
                >
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[400px] bg-white dark:bg-[#0b1120] border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white">
        <DialogHeader>
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-slate-50 dark:bg-[#111827] rounded-xl border border-slate-200 dark:border-slate-800">
            ...
            <div className="mt-2">
              ...
              {itemType === "DOCUMENT" && (
                <>
                  ...
                  {data.fileBase64 && (
                    <div className="mt-6 border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#111827] rounded-xl overflow-hidden">
                      {data.fileBase64.startsWith('data:image/') ? (
                        <div className="flex flex-col">
                          <div className="bg-slate-100 dark:bg-[#0b1120] p-2 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
                            <span className="text-xs font-medium text-slate-600 dark:text-slate-400 px-2">Image Preview</span>
                            <a 
                              href={data.fileBase64} 
                              download={data.fileName || "secure_image"}
                              className="text-xs flex items-center gap-1.5 text-emerald-600 dark:text-emerald-500 hover:text-emerald-500 dark:hover:text-emerald-400 bg-emerald-500/10 px-3 py-1.5 rounded-md transition-colors"
                            >
                        ...
                        <div className="p-6 flex flex-col items-center justify-center space-y-4 text-center">
                          ...
                          <div>
                            <h4 className="text-sm font-medium text-slate-900 dark:text-white mb-1">Secure Document Ready</h4>
              ...
              {itemType === "SECURE_NOTE" && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    {!noteUnlocked ? (
                      <div className="bg-slate-50 dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-lg p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-colors" 
                           onClick={() => {
                             if (!isAuthenticating) {
                        ...
                        {isAuthenticating ? (
                          ...
                          <Lock className="w-8 h-8 text-slate-400 dark:text-slate-500 mb-3" />
                        )}
                        <h4 className="text-sm font-medium text-slate-700 dark:text-slate-300">Note is locked</h4>
                        <p className="text-xs text-slate-500 mt-1">Click to unlock and reveal contents</p>
                      </div>
                    ) : (
                      <div className="bg-slate-50 dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-lg p-4 relative group">
                        ...
                        <button 
                          ...
                          className="absolute top-2 right-2 p-1.5 bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-md text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white transition-colors opacity-100 sm:opacity-0 sm:group-hover:opacity-100"
                          title="Lock note"
                        >
          ...
          <div className="pt-4 mt-2 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center">
            {deleteConfirm ? (
              <div className="flex items-center gap-2 w-full justify-between bg-red-500/10 p-2 rounded-lg border border-red-500/20">
                <span className="text-xs text-red-500 dark:text-red-400 font-medium px-2">Are you sure?</span>
                <div className="flex gap-2">
                  <Button variant="ghost" onClick={() => setDeleteConfirm(false)} className="h-8 px-3 text-slate-700 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white">Cancel</Button>
              ...
              <>
                <Button 
                  ...
                  className="text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-500/10 h-8 px-3 text-sm"
                >
                ...
                >
                  <Button className="bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 h-8 px-3 text-sm" onClick={() => setIsEditOpen(true)}>
